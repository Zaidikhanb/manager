...
const EXPANDED_MAX_HEIGHT = "480px";
...
function generateDemoInventory() {
  ...
  const names = ['Wireless Mouse', 'Mechanical Keyboard', 'USB-C Hub', 'Monitor 24"', 'Laptop Stand', 'Noise Cancelling Headphones', 'Webcam 4K', 'Docking Station', 'SSD 1TB', 'Power Bank', 'HDMI Cable', 'Desk Mat', 'Microphone', 'Speakers', 'Charger 65W'];
  const demo = [];
  for (let i = 1; i <= 68; i++) {
    let status = statusList[Math.floor(Math.random() * statusList.length)];
    let quantity = Math.floor(Math.random() * 30) + 1;
    let name = names[i % names.length] + (Math.floor(i / 7) > 0 ? ` Pro` : '');
    demo.push({
      id: i,
      name: `${name} ${Math.floor(i / 3) + 1}`,
      number: `SN-${Math.floor(1000 + i)}`,
      quantity: status === 'short' ? Math.floor(Math.random() * 4) + 1 : quantity,
      status: status,
      displayStatus: status,
      isShort: status === 'short' && quantity < 6
    });
  }
...
function renderStats() {
  ...
  grid.innerHTML = statsArr.map(c => `
    <div class="stat-card ${currentSection === c.key ? 'active-section' : ''}" onclick="window.setSection('${c.key}')">
      ...
      <div><div class="stat-value" style="color:${c.color}">${c.count}</div><div class="stat-label">${c.label}</div></div>
    </div>
  `).join('');
}
  ...
  for (const [key, section] of Object.entries(sectionsToShow)) {
    ...
    html += `
      <div class="section-card">
        ...
        <div class="section-body ${isCollapsed ? 'collapsed' : ''}" id="sectionBody_${key}" style="max-height: ${maxHeightInline}; overflow-y: auto; overflow-x: auto;">
          ${itemCount === 0 ? `<div class="empty-section">✨ No matching items in this section</div>` : `
            <table class="section-table">
              <thead><tr><th>Item Name</th><th>Serial / Number</th><th>Quantity</th><th>Status</th></tr></thead>
              <tbody>
                ${displayItems.map(item => `
                  <tr>
                    <td><div class="item-name"><div class="item-avatar" style="background:${section.color}22;color:${section.color}">${escapeHtml(item.name.charAt(0).toUpperCase())}</div><strong>${escapeHtml(item.name)}</strong></div></td>
                    <td>${escapeHtml(item.number || '—')}</td>
                    <td><span class="${item.isShort || (item.status === 'short' && item.quantity < 5) ? 'qty-warning' : ''}">${item.isShort ? '⚠ ' : ''}${item.quantity}</span></td>
...
window.toggleSection = function(key) {
  ...
  if (bodyDiv) {
    if (collapsedSections[key]) {
      ...
      bodyDiv.style.maxHeight = EXPANDED_MAX_HEIGHT;
