...
const SCROLLABLE_MAX_HEIGHT = "460px";
...
async function fetchWallet() {
  try {
    ...
    document.getElementById('walletBalance').textContent = '? PKR';
  }
}
...
function updateWalletDisplay(walletData) {
  ...
  if (balance < 0) {
    ...
    balanceSpan.title = `Extra used: ${Math.abs(balance)} PKR`;
  } else if (balance > 0) {
...
function renderStats() {
  ...
  document.getElementById('statsGrid').innerHTML = stats.map(c => `
    <div class="stat-card ${currentSection === c.key ? 'active-section' : ''}">
    ...
    html += `
      <div class="section-card">
        ...
        <div class="section-body ${isCollapsed ? 'collapsed' : ''}" id="sectionBody_${key}" style="max-height: ${maxHeightStyle}; overflow-y: auto; overflow-x: auto;">
          ${filteredItems.length === 0 ? `<div class="empty-section">✨ No items match your filter</div>` : `
