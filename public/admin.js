...
function updateWalletUI() {
  ...
  if (balanceChip) balanceChip.innerHTML = `$${balance.toFixed(2)}`;
  const balanceDisplay = document.getElementById('walletBalanceDisplay');
  if (balanceDisplay) balanceDisplay.innerHTML = `$${balance.toFixed(2)}`;
    ...
    listContainer.innerHTML = walletData.transactions.map(t => `
      <div style="display: flex; justify-content: space-between; padding: 8px 0; border-bottom: 1px solid rgba(255,255,255,0.05);">
        <div>
          ...
          <div style="font-size: 11px; color: var(--text3);">${new Date(t.timestamp).toLocaleString()}</div>
        </div>
        <div style="color: ${t.type === 'add' ? 'var(--green)' : 'var(--orange)'}; font-weight: 600;">
          ${t.type === 'add' ? '+' : '-'}$${Math.abs(t.amount).toFixed(2)}
        </div>
      </div>
    `).join('');
    if (!walletData.transactions.length) listContainer.innerHTML = '<div style="padding:20px; text-align:center; color:var(--text3);">No transactions yet.</div>';
  }
}
...
// Attach wallet event listeners after DOM is ready
